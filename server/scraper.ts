...
async function performHttpQuery(
  ...
  const response = await axios.post(
    `${QATAR_MOI_API}/inquiry/violation`,
    {
      inquiryType: 3,
      plateNo,
      plateCat,
  ...
  if (response.status >= 400) {
    throw new Error(`Qatar MOI API returned HTTP ${response.status}`);
  }
  ...
  return await withPlaywrightFallbackSlot(async () => {
    ...
    if (!browser) {
      return {
        success: false,
        fines: [],
        errorMessage: "تعذر الاتصال بموقع وزارة الداخلية القطرية. يرجى المحاولة مرة أخرى لاحقاً.",
      };
    }
    ...
    await page.goto(
      "https://fees2.moi.gov.qa/moipay/inquiry/violation?language=ar",
